...
export const action = async ({ request }) => {
  ...
  if (secret !== "RockhoundAlpha7799!") return json({ error: "Unauthorized" }, { status: 401 });
  
  try {
    const payload = await request.json();
    const { shop, productId, key, value, type } = payload;
    if (!shop || !productId || !key || value === undefined) return json({ error: "Missing fields" }, { status: 400 });
    ...
    if (data.data?.metafieldsSet?.userErrors?.length > 0) return json({ success: false, errors: data.data.metafieldsSet.userErrors }, { status: 400 });
