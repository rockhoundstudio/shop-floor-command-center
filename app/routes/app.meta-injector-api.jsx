...
export const action = async ({ request }) => {
  ...
  if (intent === "loadProductData") {
    try {
      ...
      if (!product) return data({ intent: "loadProductData", success: false, message: "Product not found" });
      ...
      return data({
       ...
       return data({ intent: "loadProductData", success: false, error: err.message });
    }
  }

  // ==========================================
  // TAB 3: LIVE REPAIR ENGINE (Mutates Shopify Data based on edited Repair Plan)
  // ==========================================
  if (intent === "executeRepairPlan") {
    try {
      const pieceId = formData.get("pieceId");
      const rawPlan = formData.get("repairPlan");
      const rawLegacy = formData.get("legacyKeysToRemove");
      
      if (!pieceId || !rawPlan) {
        return data({ intent: "executeRepairPlan", success: false, message: "Missing pieceId or repairPlan payload." });
      }
      ...
      // 1. Delete Legacy/Empty Keys First
      ...
      // 2. Write Edited Canonical Values
