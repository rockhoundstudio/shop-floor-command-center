...
export const action = async ({ request }) => {
  ...
  if (intent === "saveMetafields") {
    try {
      ...
      // --- HTML Description Assembly & Fallback ---
      // If the frontend does not explicitly pass a descriptionHtml string, we salvage the HTML from the generated_description payload block
      ...
      let setMetafields = payloadArray.flatMap(item => {
        ...
        if (valStr === "" || valStr.toLowerCase() === "none" || valStr.toLowerCase() === "n/a" || valStr.toLowerCase() === "null" || valStr.toLowerCase() === "undefined") {
          ...
          // Cross-clear standard Shopify taxonomy fields if this represents one 
          // e.g., if custom.chain_link_type is blank, ensure shopify.chain-link-type clears too.
        ...
        // Correctly infer block types safely
        ...
        // Apply strict data formatting overrides
        if (resolvedType === "multi_line_text_field") {
          ...
          // HUGE FIX: If Shopify maps it as a metaobject, but the generated payload string is NOT a GID
          // (e.g. passing "Silver-plated" instead of "gid://shopify/Metaobject/123"), this prevents chunk 
          // mutations from immediately blowing up the API.
      ...
      // Fix: Shopify prevents arbitrary Variant mass weight adjustments unless inventoryItemUpdate is explicitly invoked. 
      ...
      if (allErrors.length > 0) {
        return data({ intent: "saveMetafields", success: false, message: "Saved with errors.", errors: allErrors });
      }

      return data({ intent: "saveMetafields", success: true, message: "All metafields, alt text, description, and variant weights locked in.", writtenCount: setMetafields.length });
    } catch (error) {
