...
        ])
      );
      const rawMfs = { ...shopifyMfs, ...customMfs };const mfs = Object.fromEntries(  Object.entries(rawMfs).map(([k, v]) => {    if (typeof v === "string" && v.startsWith("[") && v.endsWith("]")) {      try {        const parsed = JSON.parse(v);        if (Array.isArray(parsed) && parsed.length === 1) return [k, parsed[0]];        if (Array.isArray(parsed) && parsed.length === 0) return [k, ""];      } catch {}    }    return [k, v];  }));
